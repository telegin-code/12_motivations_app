...
def app():
    st.set_page_config(page_title="Мотивационный профиль (12 факторов)", layout="wide")
        ...
        for q in QUESTIONS:
            ...
            for i, opt in enumerate(["a", "b", "c", "d"]):
                ...
                cols[i].number_input(
                    ...
                    step=1,
                    key=key,
                    value=st.session_state["answers"].get((q['num'], opt), 0),
                )

            # Обновим session_state.answers после каждого вопроса
            for opt in ["a", "b", "c", "d"]:
                key = f"q{q['num']}_{opt}"
                st.session_state["answers"][(q["num"], opt)] = form.session_state[key]
    ...
    with tab3:
        ...
        if df.empty:
            ...
            show_bar_chart(mean_factor_scores, title="Средние значения факторов (группа)")

            # Возможный будущий функционал: фильтры по подразделениям и т.п.
            st.markdown(
